...
use crate::mdata::{MData, ViewType};
...
impl Mat {
            ...
            VariantType::Array => {
                ...
                match arr.get(0).get_type() {
                    VariantType::Bool => match extract_array(&arr) {
                        Ok(values) => {
                            Some(MData::Bool(DMatrix::from_vec(
                                nrows as usize, 
                                ncols as usize, 
                                values
                            )))
    ...
    fn zeros(nrows: i64, ncols: i64, dtype: GString) -> Option<Gd<Self>> {
        ...
        DType::try_from_godot(dtype.clone())
            .map_err(|_| format!("Unknown dtype {}", dtype).into())
            .map(|dtype| Gd::from_init_fn(|base|
                Self { m: MData::zeros(nrows as usize, ncols as usize, &dtype), base, }
            ))
    ...
    fn resize(&self, nrows: i64, ncols: i64, val: Variant) -> Option<Gd<Self>> {
        ...
        self.m.resize(nrows as usize, ncols as usize, val)
            .map(|m| Gd::from_init_fn(|base|
                Self { m, base, }
            )).map_err(|err| godot_script_error!("{}", err))
            .ok()
    ...
    fn rows(&self, i: i64, size: i64) -> Option<Gd<Self>> {
        ...
        Some(Gd::from_init_fn(|base|
            Self {
                m: MData::View { mat: self.to_gd(), start, shape, view_type: self.m.get_view_type() },
                base,
            }
        ))
    }
    ...
    fn real(&self) -> Gd<Self> {
        Gd::from_init_fn(|base|
            Self {
                m: MData::View { mat: self.to_gd(), start: (0, 0), shape: self.m.shape(), view_type: ViewType::Real },
                base,
            }
        )
    }
    ...
    fn imag(&self) -> Option<Gd<Self>> {
        match self.m.dtype() {
            DType::C64 | DType::C128 => Some(Gd::from_init_fn(|base|
                Self {
                    m: MData::View { mat: self.to_gd(), start: (0, 0), shape: self.m.shape(), view_type: ViewType::Imag },
                    base,
                }
            )),
            _ => {
